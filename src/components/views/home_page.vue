...
<template>
  ...
  <div class="flex flex-wrap bg-gray-100 w-full h-screen">
    <div v-if="!sidenav" class="w-1/24 bg-white">
      <el-icon @click="sidenav = !sidenav">
        <CaretRight />
      </el-icon>
    </div>
    <div v-if="sidenav" class="w-2/12 bg-white rounded p-3 shadow-lg">
      <div class="flex items-center space-x-4 p-2 mb-5">
        <img class="h-12 rounded-full" src="http://www.gravatar.com/avatar/2acfb745ecf9d4dccb3364752d17f65f?s=260&d=mp"
          alt="James Bhatta" />
        <div>
          <h4 class="font-semibold text-xl text-gray-700 capitalize font-poppins tracking-wide">
            {{ username }}
          </h4>
          <span class="text-xl tracking-wide flex items-center space-x-1">
        ...
        <div>
          <el-icon>
            <CaretLeft @click="sidenav = !sidenav" />
          </el-icon>
        </div>
      </div>
      <ul class="space-y-2 text-xl">
        ...
        <li>
          <a href="#"
            class="flex items-center space-x-3 text-gray-700 p-2 rounded-md font-medium hover:bg-gray-200 focus:bg-gray-200">
            <span class="text-gray-600">
              <svg class="h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="w-10/12 h-full grow">
      <chat_page :username="username" />
    </div>
  </div>
</template>
<script setup>
import chat_page from "@/components/views/chat_page.vue";
import { ref } from "vue";
